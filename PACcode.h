...
class PACcode{
    ...
    public:
            ...
            _block_length(block_length), _info_length(info_length),_design_snr(design_snr),_crc_mode(crc_mode),_construction_mode(construction_mode), _S(S), _segmented_bits_length(segmented_bits_length),_segmented_crc_length(segmented_crc_length),_segmented_list_size(segmented_list_size)
        {
            ...
            _m = 7;
            // convolutional_polynomial = {1,1};
            convolutional_polynomial = {1,1,0,1,1,0,1};
            // convolutional_polynomial = {1};
            ...
            //initialize _v of 2D array of size L*(_block_length)
            _v.resize(_max_list_size);
            for (int i = 0; i < _max_list_size; i++)
            {
                _v.at(i).resize(_block_length);
            }

            //container: L*(_block_length)
            container.resize(_max_list_size);
            for (int i = 0; i < _max_list_size; i++)
            {
                container.at(i).resize(_block_length);
            }
        ...
        uint8_t check(std::vector<uint8_t> coded_bits);
    ...
    private:
        ...
        // std::vector<uint8_t> convolutional_polynomial = {1,1,0,1,1,0,1};  
        std::vector<uint8_t> convolutional_polynomial;     

        std::vector<std::vector<uint8_t>> _v;

        std::vector<std::vector<uint8_t>> container;
